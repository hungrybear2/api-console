...
(function() {
  ...
  RAML.Directives.ramlConsoleInitializer = function(ramlParserWrapper) {
    ...
    var link = function($scope, $element, $attrs, controller) {
      var start = document.location.href.indexOf('?raml=');
      if (start !== -1) {
        var search = decodeURIComponent(document.location.href.substring(start));
        controller.location = search.replace('?raml=', '');
        controller.load();
      }
    };
...
angular.module('ramlConsoleApp')
  .controller('ApiListController', ['$scope', '$location', 'ramlParserWrapper', function($scope, $location, ramlParserWrapper) {
    $scope.go = function go(filename) {
      $location.url('/').search({raml: filename});
      ramlParserWrapper.load(filename);
    };
  }]);
